...
const ChatInterface = ({ uploadedFile, fileContent }) => {
  ...
  const getMessageIcon = (type) => {
    switch (type) {
      case 'assistant':
        return <div className="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center">
    ...
    <div className="flex flex-col h-full">
      ...
      <div 
        ...
        className={`flex-1 overflow-y-auto p-4 space-y-4 ${
          isDragOver ? 'bg-primary-50 border-2 border-dashed border-primary-300' : ''
        }`}
        onDrop={handleDrop}
      ...
      >
        {isDragOver && (
          <div className="text-center py-8">
            <Link className="w-8 h-8 text-primary-500 mx-auto mb-2" />
            <p className="text-primary-600 font-medium">Drop Notion URL to share content</p>
          </div>
        )}
      ...
      <div className="border-t border-gray-200 p-4">
        ...
        <div className="mt-2 flex items-center space-x-2 text-xs text-gray-500">
          ...
          {userPolicy.autoDisconnect && (
            <span className="text-orange-600">• Auto-disconnect enabled</span>
          )}
        </div>
      </div>
    </div>
